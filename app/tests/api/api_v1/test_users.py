...
from app.tests.utils.user import random_user_dict, random_active_user_dict, random_active_superuser_dict
...
# region Create user - GET /users/{id}
...
@pytest.mark.asyncio
async def test_when_getting_user_by_id_if_user_not_exist_and_token_user_is_not_superuser_must_return_403(active_user: models.User, async_client: AsyncClient, db: AsyncSession) -> None:
    target_user_dict = random_active_user_dict()
    target_user = await crud.user.create(db=db, user_in=target_user_dict)
...
@pytest.mark.asyncio
async def test_when_getting_user_by_id_if_user_not_exist_and_token_user_is_superuser_must_return_404(active_superuser: models.User, async_client: AsyncClient, db: AsyncSession) -> None:
    target_user_dict = random_active_user_dict()
    target_user = await crud.user.create(db=db, user_in=target_user_dict)
    await crud.user.delete_by_id(db=db, id=target_user.id)
    headers = get_user_token_headers(active_superuser)
...
@pytest.mark.asyncio
async def test_when_getting_own_user_if_token_user_is_not_authenticated_must_return_401(async_client: AsyncClient) -> None:
