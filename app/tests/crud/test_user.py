...
@pytest.mark.asyncio
async def test_when_create_user_if_attribute_is_active_is_not_set_it_must_be_false(
    db: AsyncSession,
) -> None:
    user_dict = random_user_dict()
    new_user = await crud.user.create(db=db, user_in=user_dict)
...
@pytest.mark.asyncio
async def test_when_create_user_if_attribute_is_superuser_is_not_set_it_must_be_false(
    db: AsyncSession,
) -> None:
    user_dict = random_user_dict()
    new_user = await crud.user.create(db=db, user_in=user_dict)
