...
@pytest.mark.asyncio
async def test_if_get_multi_by_owner_id_return_a_list_of_tasks_for_the_given_owner(
    db: AsyncSession,
) -> None:
    owner_user = await crud.user.create(
        db=db, user_in=random_active_user_dict()
    )
    task_dict = random_task_dict() | {"owner_id": owner_user.id}
    await crud.task.create(db=db, task_in=task_dict)
    tasks = await crud.task.get_multi_by_owner_id(
        db=db, owner_id=owner_user.id, limit=1
    )
    assert tasks[0].owner_id == owner_user.id


@pytest.mark.asyncio
async def test_if_get_multi_by_owner_id_return_a_list_of_task(
    db: AsyncSession,
) -> None:
    owner_user = await crud.user.create(
        db=db, user_in=random_active_user_dict()
...
@pytest.mark.asyncio
async def test_if_get_multi_by_owner_id_return_the_correct_quantity_of_task(
    db: AsyncSession,
) -> None:
    owner_user = await crud.user.create(
        db=db, user_in=random_active_user_dict()
    ...
    for _ in range(3):
        await crud.task.create(
            db=db, task_in=random_task_dict() | {"owner_id": owner_user.id}
        )
    tasks = await crud.task.get_multi_by_owner_id(
        db=db, owner_id=owner_user.id, limit=2
    )
    assert len(tasks) == 2


@pytest.mark.asyncio
async def test_if_get_multi_by_owner_id_skip_the_correct_quantity_of_task(
    db: AsyncSession,
) -> None:
    created_tasks = []
    owner_user = await crud.user.create(
    ...
    tasks = await crud.task.get_multi_by_owner_id(
        db=db, owner_id=owner_user.id, skip=2, limit=1
    )
    assert tasks[0].id == created_tasks[2].id
